...
type SpeechConfigPayload = GetConfigPayload | UpsertConfigPayload
...
type SchoolSpeechConfigUpsertRecord = {
  school_id: string
  provider_type: string
  provider_label: string
  base_url: string
...
function formatConfig(record: SchoolSpeechConfigRecord | null) {
  if (!record) {
    return null
  }
  ...
  return {
    ...
    model: record.model,
    voicePreset: record.voice_preset,
    responseFormat: record.response_format,
...
Deno.serve(async (req) => {
  ...
  const payload = (await req.json()) as SpeechConfigPayload
  ...
  if (payload.action === 'get_config') {
    const { data, error } = await adminClient
      ...
      .select(
        'school_id, provider_type, provider_label, base_url, model, voice_preset, response_format, api_key_masked, api_key_ciphertext, enabled, updated_at',
      )
      .eq('school_id', payload.schoolId)
      .maybeSingle()
    ...
    return json({
      config: formatConfig((data as SchoolSpeechConfigRecord | null) ?? null),
    })
  }
  ...
  const voicePreset = payload.voicePreset?.trim() || null
  const responseFormat = payload.responseFormat?.trim() || 'mp3'
  ...
  let apiKeyCiphertext =
    (existingRecord as SchoolSpeechConfigRecord | null)?.api_key_ciphertext ?? null
  let apiKeyMasked =
    (existingRecord as SchoolSpeechConfigRecord | null)?.api_key_masked ?? null

  if (nextApiKey) {
    if (!encryptionSecret.trim()) {
      return json({ error: 'Speech encryption secret is not configured.' }, 500)
    }
  ...
  const upsertPayload: SchoolSpeechConfigUpsertRecord = {
  ...
  if (!(existingRecord as SchoolSpeechConfigRecord | null)?.school_id) {
  ...
  return json({
    config: formatConfig(savedConfig as SchoolSpeechConfigRecord),
  })
})
